...
const SCHEMA_SQL = `
-- 1. DISABLE RLS (To ensure data saves without complex policies)
...
alter table if exists public.market_prices disable row level security;
alter table if exists public.users disable row level security;
alter table if exists public.grievances disable row level security;
alter table if exists public.wholesale_ads disable row level security;
alter table if exists public.retail_products disable row level security;
...
alter table if exists public.vocational_courses disable row level security;
alter table if exists public.donors disable row level security;
alter table if exists public.enrolled_courses disable row level security;
...
create table if not exists public.grievances (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.wholesale_ads (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.retail_products (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.lawyers (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.exchange_rates (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.vocational_courses (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.donors (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);
create table if not exists public.enrolled_courses (id bigint generated by default as identity primary key, created_at timestamp with time zone default timezone('utc'::text, now()) not null);

-- 3. ADD COLUMNS (Safe Mode)
-- Jobs
...
-- Market Prices
alter table public.market_prices add column if not exists nameen text;
alter table public.market_prices add column if not exists namebn text;
alter table public.market_prices add column if not exists unit text;
alter table public.market_prices add column if not exists today numeric;
alter table public.market_prices add column if not exists yesterday numeric;
alter table public.market_prices add column if not exists trend text;
...
-- Others
alter table public.grievances add column if not exists location text;
alter table public.grievances add column if not exists issue text;
alter table public.grievances add column if not exists type text;
alter table public.grievances add column if not exists image text;
alter table public.grievances add column if not exists "user" text;
alter table public.grievances add column if not exists status text default 'Pending';
alter table public.grievances add column if not exists date text;
alter table public.grievances add column if not exists severity text;

alter table public.wholesale_ads add column if not exists product text;
alter table public.wholesale_ads add column if not exists quantity text;
alter table public.wholesale_ads add column if not exists price text;
alter table public.wholesale_ads add column if not exists seller text;
alter table public.wholesale_ads add column if not exists sellertype text;
alter table public.wholesale_ads add column if not exists location text;
alter table public.wholesale_ads add column if not exists date text;
alter table public.wholesale_ads add column if not exists status text default 'Pending';

alter table public.retail_products add column if not exists nameen text;
alter table public.retail_products add column if not exists namebn text;
alter table public.retail_products add column if not exists price numeric;
alter table public.retail_products add column if not exists unit text;
alter table public.retail_products add column if not exists img text;
alter table public.retail_products add column if not exists category text;
alter table public.retail_products add column if not exists stock text default 'Available';

alter table public.lawyers add column if not exists name text;
alter table public.lawyers add column if not exists speciality text;
alter table public.lawyers add column if not exists phone text;
alter table public.lawyers add column if not exists location text;
alter table public.lawyers add column if not exists status text default 'Active';

alter table public.exchange_rates add column if not exists currency text;
alter table public.exchange_rates add column if not exists rate numeric;
alter table public.exchange_rates add column if not exists trend text;

alter table public.vocational_courses add column if not exists title text;
alter table public.vocational_courses add column if not exists titlebn text;
alter table public.vocational_courses add column if not exists category text;
alter table public.vocational_courses add column if not exists duration text;
alter table public.vocational_courses add column if not exists fee numeric;
alter table public.vocational_courses add column if not exists image text;
alter table public.vocational_courses add column if not exists status text default 'Active';
...
alter table public.donors add column if not exists "group" text;
alter table public.donors add column if not exists lastdonation text;

alter table public.enrolled_courses add column if not exists title text;
alter table public.enrolled_courses add column if not exists titlebn text;
alter table public.enrolled_courses add column if not exists enrolleddate text;
alter table public.enrolled_courses add column if not exists progress numeric;
alter table public.enrolled_courses add column if not exists status text;

-- 4. GRANT PERMISSIONS (Critical for anon access)
...
export const AdminWebsiteManage = () => {
  ...
  const handleTestInsert = async () => {
      ...
      try {
          ...
          if (error) {
              setTestResult(`Insert Failed: Code ${error.code} - ${error.message}. Hint: ${error.hint || 'None'}`);
          } else {
            ...
            {!isSupabaseConfigured ? (
                ...
                <div className="text-center py-6">
                    <div className="flex gap-4 justify-center mb-6">
                        ...
                        <Button onClick={handleTestInsert} variant="outline" className="flex items-center gap-2 bg-blue-50 hover:bg-blue-100 text-blue-700 border-blue-200">
                            <PlusCircle size={16} /> Test Data Insert
                        </Button>
                    </div>
                    
                    {testResult && (
                        <div className={`mb-6 p-4 rounded-xl border text-left text-sm font-mono whitespace-pre-wrap break-all ${testResult.startsWith('Success') || testResult.startsWith('Insert Success') ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                            {testResult}
                        </div>
                    )}
                    ...
                    <div className="bg-gray-900 text-white p-6 rounded-2xl border border-gray-800 text-left">
                        ...
                        <p className="text-sm text-gray-300 mb-4 leading-relaxed">
                            If data isn't saving, run this SQL. It <strong>disables Row Level Security (RLS)</strong> and grants full permissions to ensure data saves correctly.
                        </p>
