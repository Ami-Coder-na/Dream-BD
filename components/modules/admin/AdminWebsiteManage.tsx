...
-- 3. REQUESTS (UPDATED: Added 'excerpt', 'image', 'readtime' for blogs)
create table if not exists public.requests (
  ...
  image text, content text, readtime text, posteddate text, excerpt text
);
...
drop policy if exists "Enable all access for all users" on public.grievances;
create policy "Enable all access for all users" on public.grievances for all using (true) with check (true);

-- 9. RETAIL PRODUCTS
create table if not exists public.retail_products (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  nameen text, namebn text, price numeric, unit text, img text, category text, stock text default 'Available'
);
alter table public.retail_products enable row level security;
drop policy if exists "Enable all access for all users" on public.retail_products;
create policy "Enable all access for all users" on public.retail_products for all using (true) with check (true);

-- 10. WHOLESALE ADS
create table if not exists public.wholesale_ads (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  product text, quantity text, price text, seller text, sellertype text, location text, date text, status text default 'Active'
);
alter table public.wholesale_ads enable row level security;
drop policy if exists "Enable all access for all users" on public.wholesale_ads;
create policy "Enable all access for all users" on public.wholesale_ads for all using (true) with check (true);

-- 11. LAWYERS
create table if not exists public.lawyers (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  name text, speciality text, phone text, location text, status text default 'Active'
);
alter table public.lawyers enable row level security;
drop policy if exists "Enable all access for all users" on public.lawyers;
create policy "Enable all access for all users" on public.lawyers for all using (true) with check (true);
...
drop policy if exists "Enable all access for all users" on public.exchange_rates;
create policy "Enable all access for all users" on public.exchange_rates for all using (true) with check (true);

-- 13. VOCATIONAL COURSES
create table if not exists public.vocational_courses (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  title text, titlebn text, category text, duration text, fee numeric, image text, status text default 'Active'
);
alter table public.vocational_courses enable row level security;
drop policy if exists "Enable all access for all users" on public.vocational_courses;
create policy "Enable all access for all users" on public.vocational_courses for all using (true) with check (true);
...
drop policy if exists "Enable all access for all users" on public.donors;
create policy "Enable all access for all users" on public.donors for all using (true) with check (true);

-- 15. ENROLLED COURSES
create table if not exists public.enrolled_courses (
  id bigint generated by default as identity primary key,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  title text, titlebn text, enrolleddate text, progress numeric, status text
);
alter table public.enrolled_courses enable row level security;
drop policy if exists "Enable all access for all users" on public.enrolled_courses;
create policy "Enable all access for all users" on public.enrolled_courses for all using (true) with check (true);
...
drop policy if exists "Enable all access for all users" on public.contact_messages;
create policy "Enable all access for all users" on public.contact_messages for all using (true) with check (true);
...
const HARDCODED_KEY = '${dbKey || 'YOUR_ANON_KEY_HERE'}';`}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center gap-3 bg-gray-50">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-900">System & Announcements</h3>
            <p className="text-xs text-gray-500">Manage site availability and global notices</p>
          </div>
        </div>
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-red-50 border border-red-100 rounded-xl">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white rounded-full text-red-500 shadow-sm"><Power size={20} /></div>
              <div>
                <h4 className="font-bold text-gray-900">Maintenance Mode</h4>
                <p className="text-xs text-gray-500">Enable to block public access (Admins can still login)</p>
              </div>
            </div>
            <ToggleSwitch 
              label="" 
          ...
          <div className="border border-gray-100 rounded-xl p-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-orange-100 text-orange-600 rounded-lg"><Megaphone size={20} /></div>
                <div>
                  <h4 className="font-bold text-gray-900">Top Announcement Bar</h4>
                  <p className="text-xs text-gray-500">Show a notice at the top of every page</p>
                </div>
              </div>
              <ToggleSwitch 
                label="" 
