            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                        ...
                        {view === 'edit_job' ? 'Edit Job Post' : 'Add New Job Post'}
                    </h3>
                ...
                <form onSubmit={handleJobSubmit} className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-100" value={jobForm.title} onChange={e => setJobForm({...jobForm, title: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-1">Company</label>
                            <input required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-100" value={jobForm.company} onChange={e => setJobForm({...jobForm, company: e.target.value})} />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-100" value={jobForm.location} onChange={e => setJobForm({...jobForm, location: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-1">Salary</label>
                            <input className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-100" value={jobForm.salary} onChange={e => setJobForm({...jobForm, salary: e.target.value})} placeholder="e.g. 25k-30k" />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                    ...
                    <div>
                        ...
                        <textarea rows={5} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-purple-100 resize-none" value={jobForm.description} onChange={e => setJobForm({...jobForm, description: e.target.value})}></textarea>
                    </div>
                    <Button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 text-white py-3.5 font-bold">
                        {view === 'edit_job' ? 'Update Job' : 'Post Job Now'}
                    </Button>
                </form>
            </div>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                        ...
                        {view === 'edit_blog' ? 'Edit Blog Article' : 'Write New Blog'}
                    </h3>
                    <button onClick={() => setView('list')} className="text-gray-400 hover:text-red-500"><X size={20}/></button>
                </div>
                <form onSubmit={handleBlogSubmit} className="p-8 space-y-6">
                    <div>
                        ...
                        <input required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-100" value={blogForm.title} onChange={e => setBlogForm({...blogForm, title: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-1">Category</label>
                            <select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer" value={blogForm.category} onChange={e => setBlogForm({...blogForm, category: e.target.value})}>
                                <option>Agriculture</option><option>Health</option><option>Education</option><option>Technology</option><option>General</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-1">Cover Image</label>
                            <div className="flex gap-2">
                                <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} className="flex-1 border-dashed">
                                    {blogForm.image ? 'Change Image' : 'Upload Image'}
                                </Button>
                                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
                                {blogForm.image && <div className="w-10 h-10 rounded-lg border overflow-hidden"><img src={blogForm.image} className="w-full h-full object-cover" /></div>}
                            </div>
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Content</label>
                        <textarea rows={10} required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-100 resize-none" value={blogForm.content} onChange={e => setBlogForm({...blogForm, content: e.target.value})}></textarea>
                    </div>
                    <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 font-bold">
                        {view === 'edit_blog' ? 'Update Article' : 'Publish Article'}
                    </Button>
                </form>
            </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
           <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8 border-b border-gray-50 pb-6">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                <Button onClick={handleOpenCreateJob} className="flex-1 md:flex-none bg-purple-600 hover:bg-purple-700 text-white flex items-center justify-center gap-2 px-6 shadow-md">
                    <Briefcase size={18} /> Add Job
                </Button>
                <Button onClick={handleOpenCreateBlog} className="flex-1 md:flex-none bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center gap-2 px-6 shadow-md">
           ...
           {activeTab !== 'requests' && (
             <div className="flex flex-col md:flex-row gap-4 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="relative flex-1">
                   ...
                   <input 
                      type="text" 
                      placeholder={activeTab === 'jobs' ? "Search jobs or companies..." : "Search blog titles or authors..."}
                ...
                <div className="flex gap-3 flex-wrap">
                    <div className="relative">
                       <select 
                          className="pl-9 pr-8 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-brand-500 outline-none appearance-none cursor-pointer"
                    ...
                    {(contentSearch || categoryFilter !== 'All' || jobTypeFilter !== 'All') && (
                       <button 
                 ...
                 <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-50/50 border-b border-gray-100">
                       <tr>
                         ...
                         <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">{activeTab === 'jobs' ? 'COMPANY' : 'AUTHOR'}</th>
                         <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">CATEGORY</th>
                         {activeTab === 'jobs' && <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">TYPE</th>}
                         <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">ACTIONS</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                       {(activeTab === 'jobs' ? filteredJobs : filteredBlogs).map((item: any) => (
                          <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                             ...
                             <td className="p-4">
                                <span className="bg-gray-100 text-gray-700 px-2.5 py-1 rounded text-[10px] font-bold border border-gray-200">
                                   {item.category}
                                </span>
                             </td>
                             {activeTab === 'jobs' && (
                               <td className="p-4">
                                 <span className="bg-blue-50 text-blue-700 px-2.5 py-1 rounded text-[10px] font-bold border border-blue-100">
                                   {item.type}
                                 </span>
                               </td>
                             )}
                       ...
                       {(activeTab === 'jobs' ? filteredJobs : filteredBlogs).length === 0 && (
                          <tr>
                             <td colSpan={5} className="p-12 text-center text-gray-400 font-medium">
                                <div className="flex flex-col items-center gap-3">
                                   <Search size={32} className="opacity-20" />
                                   <p>No matches found. Try changing your search or filters.</p>
                                </div>
                             </td>
                          </tr>
                       )}
